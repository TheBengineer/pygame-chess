...
class Chess(object):
    ...
    def play_turn(self):
        ...
        turn_text = small_font.render("", True, white_color)
        if self.turn["black"]:
    ...
    def draw_board(self, draw_surface, x, y):
        self.screen.blit(draw_surface, self.board_locations[x][y])
        if len(self.moves) > 0:
            for move in self.moves:
                x_coord = move[0]
                y_coord = move[1]
                if 0 <= x_coord < 8 and 0 <= y_coord < 8:
                    self.screen.blit(draw_surface, self.board_locations[x_coord][y_coord])
        ...
        for val in self.piece_location.values():
            for value in val.values():
                ...
                if value[1] and len(value[0]) > 5:
                    ...
                    if value[0][:5] == "black":
                        self.draw_board(surface, piece_coord_x, piece_coord_y)
                    # if the piece selected is a white piece
                    elif value[0][:5] == "white":
                        self.draw_board(surface1, piece_coord_x, piece_coord_y)
            ...
            # calculate moves for pawn
                ...
                if piece_name == "black_pawn":
                    if y_coord + 1 < 8:
                        ...
                        # pawns cannot move when blocked by another pawn
            ...
            for i in self.moves:
                if i == [x, y]:
                    if (p[0][:5] == turn) or len(p[0]) == 0:
                        ...
                        self.capture_piece(turn, [column_char, row_no], [x, y])
    ...
    def validate_move(self, destination):
        des_col_char = chr(97 + destination[0])
        des_row_no = 8 - destination[1]
        ...
        for k in self.piece_location.keys():
            for key in self.piece_location[k].keys():
                ...
                if board_piece[1]:
                    ...
                    self.piece_location[des_col_char][des_row_no][0] = piece_name
                    ...
                    des_location = des_col_char + str(des_row_no)
